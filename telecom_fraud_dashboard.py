...
class TelecomFraudDashboard:
    ...
    def verify_article_authenticity(self, article):
        ...
        is_authentic = authenticity_score >= 50 and len(issues) <= 2
    ...
    def export_to_csv(self, articles):
        """Export articles to CSV with title, fraud types, and URL"""
        export_data = []
        
        for article in articles:
            fraud_types_str = ', '.join(article['fraud_types']) if article['fraud_types'] else 'None'
            export_data.append({
                'Title': article['title'],
    ...
    def process_news_data(self, all_articles):
        ...
        for article in all_articles:
            ...
            # Only include authentic articles
            if verification_result['is_authentic']:
                ...
                if detected_fraud_types:
                    ...
                    verification_stats['authentic'] += 1
                else:
                    verification_stats['rejected'] += 1
            else:
                if verification_result['authenticity_score'] >= 30:
...
def main():
    ...
    if st.sidebar.button("🔄 Fetch Latest News", type="primary"):
        with st.spinner("Fetching news from multiple sources..."):
            ...
            for i, query in enumerate(queries[:20]):  # Limit to 20 queries for performance
                # Fetch from NewsAPI
                ...
                progress_bar.progress((i + 1) / min(len(queries), 20))
            ...
            if all_articles:
                ...
                for article in processed_articles:
                    ...
                    for seen_title in seen_titles:
                        if len(title_words.intersection(set(seen_title.split()))) / len(title_words.union(set(seen_title.split()))) > 0.7:
                            is_duplicate = True
                            break
                    if not is_duplicate:
    ...
    if 'articles' in st.session_state and st.session_state.articles:
        ...
        with col1:
            st.metric("Total Verified Articles", len(articles))
        
        with col2:
            fraud_types = [fraud_type for article in articles for fraud_type in article['fraud_types']]
        ...
        with col4:
            avg_sentiment = np.mean([article['sentiment'] for article in articles])
        ...
        with col5:
            if 'verification_stats' in st.session_state:
                ...
                rejected_count = stats['suspicious'] + stats['rejected']
                st.metric("Articles Filtered", rejected_count, help="Suspicious or fraudulent articles removed")
        ...
        with col1:
            st.write(f"Export {len(articles)} verified articles to CSV format")
        ...
        with col1:
            selected_fraud_types = st.multiselect(
                ...
                options=list(set([fraud_type for article in articles for fraud_type in article['fraud_types']])),
                default=[]
            )
        ...
        if selected_fraud_types:
            filtered_articles = [
                ...
                if any(fraud_type in article['fraud_types'] for fraud_type in selected_fraud_types)
            ]
        ...
        st.write(f"Showing {len(filtered_articles)} verified articles")
        
        for i, article in enumerate(filtered_articles[:50]):  # Limit to 50 articles for performance
            ...
            with st.container():
                ...
                <div class="news-card">
                    ...
                    <div>
                        {''.join([f'<span class="fraud-type">{fraud_type}</span>' for fraud_type in article['fraud_types']])}
                    </div>
                </div>
                """, unsafe_allow_html=True)
        ...
        4. **✅ Link Verification** - Automatically verify article URLs and filter suspicious content
        5. **📤 CSV Export** - Export news titles, fraud types, and URLs to CSV format
        6. **🔧 Sorting and Filtering** - Organize news by date, source, or fraud type
        7. **📱 Responsive Interface** - Clean, modern web interface
        ...
        **How to get started:**
        ...
        4. Use the "📥 Download CSV" button to export your results
        
        **Note:** Only articles matching your managed fraud types will be displayed.
        """)
